...
function migrateLegacyBundle(storagePrefix: string, accountUsername: string) {
  try {
    ...
    if (accountUsername !== 'Admin') return;
    const targetCards = `${storagePrefix}cards`;
    if (localStorage.getItem(targetCards)) return;
    const legCards = localStorage.getItem('fm_cards');
    if (!legCards) return;
    localStorage.setItem(targetCards, legCards);
    const legProg = localStorage.getItem('fm_progress');
    if (legProg) localStorage.setItem(`${storagePrefix}progress`, legProg);
    const legProf = localStorage.getItem('fm_profile');
    if (legProf) localStorage.setItem(`${storagePrefix}profile`, legProf);
...
function useScopedLocalStorage<T>(
  ...
  const [value, setValue] = useState<T>(() => {
    migrateLegacyBundle(storagePrefix, accountUsername);
    try {
...
export function StoreProvider({
  ...
  children: ReactNode;
  /** From useAuth — encodes username for stable localStorage keys */
  ...
  const [profile, setProfile] = useScopedLocalStorage<UserProfile>(prefix, 'profile', {
  ...
  const getCardsByGrade = useCallback((grade: Grade | 'all') => {
    ...
    return cards.filter((c: VocabCard) => c.grade === grade);
  }, [cards]);

  const getDueCards = useCallback((grade: Grade | 'all' = 'all') => {
    const now = Date.now();
    return getCardsByGrade(grade).filter((card: VocabCard) => {
  ...
  const getNewCards = useCallback((grade: Grade | 'all' = 'all') => {
    return getCardsByGrade(grade).filter((card: VocabCard) => !progress[card.id]);
  }, [getCardsByGrade, progress]);

  const getLearnedCards = useCallback((grade: Grade | 'all' = 'all') => {
    return getCardsByGrade(grade).filter((card: VocabCard) => progress[card.id]?.learned);
  }, [getCardsByGrade, progress]);
  ...
  const reviewCard = useCallback((
    ...
    setProgress((prev: Record<string, CardProgress>) => {
    ...
    if (typeof window !== 'undefined') window.dispatchEvent(new CustomEvent('fm-user-data-changed'));
    addXP(correct ? 10 : 3);
  }, [setProgress, addXP]);
  ...
  const importCards = useCallback((text: string) => {
    ...
    if (newCards.length > 0) setCards((prev: VocabCard[]) => [...prev, ...newCards]);
  ...
  const exportCards = useCallback((grade: Grade | 'all' = 'all') => {
    ...
    const lines = exportable.map((c: VocabCard) => {
  ...
  const getGradeStats = useCallback((grade: Grade) => {
    const gradeCards = cards.filter((c: VocabCard) => c.grade === grade);
    const total = gradeCards.length;
    const learned = gradeCards.filter((c: VocabCard) => progress[c.id]?.learned).length;
    const reviewed = gradeCards.filter((c: VocabCard) => progress[c.id]?.totalAttempts);
    const due = gradeCards.filter((c: VocabCard) => {
      const p = progress[c.id];
      return p && p.nextReview <= Date.now();
    }).length;
    let accuracy = 0;
    if (reviewed.length > 0) {
      const totalCorrect = reviewed.reduce((s: number, c: VocabCard) => s + (progress[c.id]?.correctAttempts || 0), 0);
      const totalAttempts = reviewed.reduce((s: number, c: VocabCard) => s + (progress[c.id]?.totalAttempts || 0), 0);
  ...
  const getWeakCards = useCallback((grade: Grade | 'all' = 'all', limit = 10) => {
    const gradeCards = getCardsByGrade(grade).filter((c: VocabCard) => progress[c.id]?.totalAttempts);
    return gradeCards
      .sort((a: VocabCard, b: VocabCard) => getAccuracy(a.id) - getAccuracy(b.id))
      .slice(0, limit);
  ...
  const getStrongCards = useCallback((grade: Grade | 'all' = 'all', limit = 10) => {
    const gradeCards = getCardsByGrade(grade).filter((c: VocabCard) => progress[c.id]?.totalAttempts);
    return gradeCards
      .sort((a: VocabCard, b: VocabCard) => getAccuracy(b.id) - getAccuracy(a.id))
      .slice(0, limit);
  }, [getCardsByGrade, progress, getAccuracy]);
  ...
  const getOverallAccuracy = useCallback((grade: Grade | 'all' = 'all') => {
    ...
    const tc = gradeCards.reduce((s: number, c: VocabCard) => s + (progress[c.id]?.correctAttempts || 0), 0);
    const ta = gradeCards.reduce((s: number, c: VocabCard) => s + (progress[c.id]?.totalAttempts || 0), 0);
  ...
  const getOverallRetention = useCallback((grade: Grade | 'all' = 'all') => {
    const gradeCards = getCardsByGrade(grade).filter((c: VocabCard) => progress[c.id]?.totalAttempts);
    if (gradeCards.length === 0) return 0;
    const scores = gradeCards.map((c: VocabCard) => getRetentionScore(c.id));
  ...
  const deleteCard = useCallback((cardId: string) => {
    setCards((prev: VocabCard[]) => prev.filter((c: VocabCard) => c.id !== cardId));
    setProgress((prev: Record<string, CardProgress>) => {
      const copy = { ...prev };
      delete copy[cardId];
      return copy;
