...
export function QuizEngine({ selectedGrade }: Props) {
  ...
  useEffect(() => {
    if (!quizActive || quizComplete || showResult) return;
    const question = questions[currentQ];
    if (!question || question.type !== 'listening') return;
    speechSynthesis.cancel();
    const timer = window.setTimeout(() => {
      void speakFrenchWhenReady(question.card.french);
    }, 450);
    return () => {
      window.clearTimeout(timer);
      speechSynthesis.cancel();
    };
  }, [quizActive, quizComplete, showResult, currentQ, questions]);
        ...
        <div className="text-center mb-8">
          {q.type === 'listening' ? (
            <div>
              ...
              <button
                onClick={() => void speakFrenchWhenReady(q.card.french)}
